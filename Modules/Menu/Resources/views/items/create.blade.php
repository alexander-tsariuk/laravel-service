...
@section('content')
    <div class="row">
        <div class="col-12">
            <form id="editItem" action="{{ route('dashboard.menu.items.store', ['itemId' => $itemId]) }}" method="POST">
                @csrf
                <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <select class="custom-select form-control-border {{ $errors->has('menuId') ? 'is-invalid' : '' }}" name="menuId" required>
                                ...
                                @if(isset($menus) && !empty($menus))
                                    @foreach($menus as $menu)
                                        <option value="{{ $menu->id }}" {{ $menu->id == $itemId ? 'selected' : '' }}>{{ $menu->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                            ...
                            <select class="custom-select form-control-border {{ $errors->has('parentId') ? 'is-invalid' : '' }}" name="parentId" required>
                                ...
                                @if(isset($menuItems) && !empty($menuItems))
                                    @foreach($menuItems as $menuItem)
                                        <option value="{{ $menuItem->id }}">{{ $menuItem->translation->label }}</option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="custom-select form-control-border {{ $errors->has('url') ? 'is-invalid' : '' }}" name="url" required>
